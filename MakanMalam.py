...
# Load dataset
dataset = pd.read_csv("Resep_Makan_Malam.csv")
...
def hitung_kebutuhan_faktor(mealtime):
    if mealtime == "makan malam":
        faktor = 0.25
    elif mealtime == "selingan makan malam":
        faktor = 0.05
    else:
...
def recommend():
    ...
    target = hitung_kebutuhan_nutrisi("makan malam", AKEi, penyakit_input, jenis_kelamin, alergi)
    if target is None:
