...
# Load dataset
dataset = pd.read_csv("Resep_Makan_Siang.csv")
...
def hitung_kebutuhan_faktor(mealtime):
    if mealtime == "makan siang":
        faktor = 0.30
    elif mealtime == "selingan makan siang":
        faktor = 0.10
    else:
...
def recommend():
    ...
    target = hitung_kebutuhan_nutrisi("makan siang", AKEi, penyakit_input, jenis_kelamin, alergi)
    if target is None:
